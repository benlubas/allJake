...
function getAngleDeg(p1, p2) {
	let q = getQuadrent(p1, p2); 
  let angleRad = Math.atan((p1.y - p2.y) / (p1.x - p2.x));
  let angleDeg = angleRad * 180 / Math.PI;
  return (adjustAngle(angleDeg, q));
}
...
const WASD_Move = function(speed){
	...
	if(x === 0 || y === 0)
		...
		return new Point(speed * x * (Math.sqrt(2) / 2), speed * y * (Math.sqrt(2) / 2)); 
}
